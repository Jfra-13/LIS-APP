...
class EnfermeriaRequiredMixin(UserPassesTestMixin):
    ...
    def handle_no_permission(self):
        messages.error(self.request, 'Solo el personal de Enfermeria puede registrar triajes.')
...
class TriajeCreateView(EnfermeriaRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        self.paciente = get_object_or_404(Paciente, pk=kwargs['paciente_pk'], estado='activo')
        return super().dispatch(request, *args, **kwargs)

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['paciente'] = self.paciente
        context['title'] = f'Triaje inicial: {self.paciente.nombres} {self.paciente.apellidos}'
        return context

    def form_valid(self, form):
        try:
            self.object = form.save(paciente=self.paciente, usuario_enfermeria=self.request.user)
